...
def main():
    ...
    mode = st.sidebar.radio(
        "Select Mode",
        ["🎵 Predict from Sample", "🎤 Predict from Upload", "📊 Batch Analysis"],
        index=0
    )
    ...
    if mode == "🎵 Predict from Sample":
        st.markdown("## 🎵 Select a Sample Song")
        ...
        if st.button("🎯 Analyze Emotion", type="primary"):
            with st.spinner("🧠 Analyzing musical features and predicting emotion..."):
                try:
                    # Make prediction
                    ...
                    tab1, tab2, tab3 = st.tabs(["🌊 Waveform", "🎨 Spectrogram", "🎼 Features"])
    ...
    # ========================================================================
        ...
        for emotion, songs in audio_catalog.items():
            if songs:
                with st.expander(f"{EMOTION_EMOJIS.get(emotion, '🎵')} {emotion} ({len(songs)} songs)"):
        ...
        if selected_songs:
            st.info(f"📌 Selected {len(selected_songs)} songs for analysis")
